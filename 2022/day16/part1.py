...
def doBranch(tunnels, minutes, released, start, opened, paths):
    maxReleased = released
    if minutes > 0:
        for end in tunnels:
            if tunnels[end]['rate'] > 0 and end not in opened:
                path, dist = paths[start][end]
                
                if len(path) > 0 and minutes > len(path)+1:                  
                    released += tunnels[end]['rate'] * (minutes-len(path)-1)
                    opened.add(end)            
                    rtn = doBranch( tunnels, minutes-len(path)-1, released, end, opened, paths)
                    released -= tunnels[end]['rate'] * (minutes-len(path)-1)
                    opened.remove(end)
...
# Pre-calculate all the shortest paths
shortestPaths = {}
for start in tunnels:
    to = {}
    for end in tunnels:  
        to[end] = djikstra(tunnels, start, end)
    shortestPaths[start] = to
...
start = 'AA'
opened.add(start)
print(doBranch(tunnels, minutes, 0, "AA", opened,shortestPaths))
